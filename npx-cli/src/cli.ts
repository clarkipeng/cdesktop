    ...
    .then((latest) => {
      if (latest && latest !== CLI_VERSION) {
        setTimeout(() => {
          ...
          console.log(`Run: npx cdesktop@latest`);
        }, 2000);
      }
    })
    .catch(() => {});
...
async function runMcp(args: string[]): Promise<void> {
  await extractAndRun("cdesktop-mcp", (bin) => {
...
async function runReview(args: string[]): Promise<void> {
  await extractAndRun("cdesktop-review", (bin) => {
...
async function main(): Promise<void> {
  ...
  cli
    .command("[...args]", "Launch the local cdesktop app")
